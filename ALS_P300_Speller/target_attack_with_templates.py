...
interval = 250  # ms. The time between two intensifications.
...
epsilon = 0.8  # to control the SNR of EEG with noise indirectly.
Fs = 256  # Hz
nb_rounds = 10  # How many rounds are used to obtain the character.
# ========================================================
perturb_time = 2 * interval

model_dir = os.path.join('runs', model_name, '{}'.format(subject))
...
for target_char in target_char_list:
    ...
    clean_acc = np.mean(np.where(np.array(predict_chars)==np.array(char), 1., 0.))
    print('Attacking success rate: {}'.format(acc))
    acc_dict[target_char] = acc
    clean_acc_dict[target_char] = clean_acc

np.savez(acc_file_path, acc=acc_dict, clean_acc=clean_acc_dict)
